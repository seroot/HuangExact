...
def Multi(x, A, B, C, D, E):
    return A + B*np.cos(x) + C*np.cos(x)**2 + D*np.cos(x)**3 + E*np.cos(x)**4
...
def generatepolymer( chainlength, BondLengths , Angles, CUM2, Box_Length):
    ...
    Position_M[0] = [ (Box_Length - 3*chainlength) - (Box_Length - 2*3*chainlength)*random.random(), (Box_Length - 3*chainlength) - (Box_Length - 2*3*chainlength)*random.random(), (Box_Length - 3*chainlength) - (Box_Length - 2*3*chainlength)*random.random()]
...
def Gen_Many_Lin_Polymers( ChainLength, NumChains, r0, th0, CDF, Sigma, Box_Length):
    ...
    while (NChains < NumChains):
        ...
        if (Bool):
            Position = np.concatenate((Position, PositionNew), axis=0)
...
def Gen_Many_Polymers( ChainLength, NumChains, BondLengths, Angles, CUM2, SigmaM_M, Box_Length):
    ...
    while (NChains < NumChains):
        ...
        if (Bool):
            Position_M = np.concatenate((Position_M, PositionNew_M), axis=0)
            Position_S1 = np.concatenate((Position_S1, PositionNew_S1), axis=0)
            Position_S2 = np.concatenate((Position_S2, PositionNew_S2), axis=0)
            Bases = np.concatenate((Bases, BasesNew), axis=0)
            NChains += 1
            print "Polymer Deposited %d" %NChains
